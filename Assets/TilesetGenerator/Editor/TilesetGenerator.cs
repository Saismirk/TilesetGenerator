...
namespace TilesetGenerator {
    public class TilesetGenerator {
        ...
        public async Task CreateTilesetFromBaseTexture(Texture2D input, string filename, TilesetGeneratorEditorWindow tileGenWindow,
        ...
        private async Task FirstPass(CancellationToken ct) {
            ...
            var progressStep = pasteTasks.Length > 0 ? 1f / pasteTasks.Length : 1;
        ...
        private async Task SecondPass(CancellationToken ct = default) {
            ...
            var progressStep = pasteTasks.Length > 0 ? 0.5f / pasteTasks.Length : 0.5f;
            foreach (var task in pasteTasks.Select((value, index) => (value, index))) {
                if (ct.IsCancellationRequested) break;
                await task.value.Invoke();
                EditorUtility.DisplayProgressBar("Tileset Generation",
        ...
        private async Task FinishUp(string fileName = "", CancellationToken ct = default) {
            ...
            var name = Utils.GetSanitizedFileName(fileName, t);
            _playerSetName = name;

            _txtrPath = FULL_PATH + _playerSetName + Utils.FILE_TYPE;
            var bytes = _outTex.EncodeToPNG();
            await File.WriteAllBytesAsync(Application.dataPath + FULL_PATH + _playerSetName + Utils.FILE_TYPE, bytes, ct);
            AssetDatabase.ActiveRefreshImportMode = AssetDatabase.RefreshImportMode.InProcess;
            AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
            await Task.Delay(1000, ct);
            _finalTexture = await LoadAsset<Texture2D>("ExportTilemap/" + _playerSetName, ct);
            if (!_finalTexture) {
        ...
        private void GetChildTiles() {
            var sprites = Resources.LoadAll<Sprite>("ExportTilemap/" + _playerSetName);
            spriteList.Clear();
